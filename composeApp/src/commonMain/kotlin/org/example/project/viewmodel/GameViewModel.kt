...
@Serializable
data class GameData(
    val matchHistory: List<GameMatch>,
    val currentMatch: GameMatch? = null
)
...
class GameViewModel : ViewModel() {
    ...
    fun onKeypadPress(playerId: Int, key: String) {
        ...
        val newScore = when (key) {
            ...
            "+/-" -> {
                ...
                if (currentScore == "0") key else currentScore + key
            }
        }
    ...
    fun tinhDiemDuKien(playerId: Int): Int {
        ...
        if (isSimpleInput) {
            return nhap.manualScores[playerId]?.toIntOrNull() ?: 0
        }
        
        if (nhap.anTrangIds.isNotEmpty()) {
            return if (nhap.anTrangIds.contains(playerId)) 3 else 0
        }
    ...
    fun chotSoLuuVan() {
        val nhap = toNhap.value
        val hopLe = if (isSimpleInput) {
            nhap.manualScores.values.any { (it.toIntOrNull() ?: 0) != 0 }
        } else {
            nhap.anTrangIds.isNotEmpty() || nhap.denIds.isNotEmpty() || nhap.rankMap.size == 4
        }
        ...
        players.forEach { p ->
            bienDongDiem[p.id] = tinhDiemDuKien(p.id)
        }
    ...
    private fun luuVanVaoSo(diemMoi: Map<Int, Int>) {
        val vanMoi = GameRound(
            roundNumber = danhSachVanDau.size + 1,
            scoreChanges = diemMoi
        )
        danhSachVanDau.add(0, vanMoi)
        diemMoi.forEach { (id, diem) ->
            val index = players.indexOfFirst { it.id == id }
            if (index != -1) {
                players[index] = players[index].copy(score = players[index].score + diem)
            }
        }
